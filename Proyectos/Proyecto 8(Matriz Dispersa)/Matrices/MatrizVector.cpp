...
/*
 void MatrizVector::definir_valor_repetido(int nuevo_rep) {
 bool validar = true;
 for (int i = 0; i < nt; i++) {
 if (vd[i] == nuevo_rep) {
 validar = false;
 ...
 // Aqui se metos los valores anteriores repetidos
 for (int i = 0; i < df; i++) {
 for (int j = 0; j < dc; j++) {
 int dato = elemento(i + 1, j + 1);
 if (dato == rep) {
 vf[nt] = i + 1;
 vc[nt] = j + 1;
 vd[nt] = dato;
