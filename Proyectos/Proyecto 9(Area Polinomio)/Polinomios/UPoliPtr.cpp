...
/*
 float PoliPtr::area(float a, float b) {
 float* exp = new float[nt];
 float* coef = new float[nt];
 for (int i = 0; i < nt; i++) {
 int ex = exponente(i);
 ...
 return abs(evaluaB - evaluaA);//valor absoluto
 } */
...
float PoliPtr::area(float a, float b) {
	...
	float valorEnB = 0;
	for (int i = 0; i < nt; i++) {
		valorEnA = valorEnA + coef[i] * pow(a, exp[i]);
		valorEnB = valorEnB + coef[i] * pow(b, exp[i]);
	}
	float ar = valorEnB - valorEnA;
	if (ar < 0)
		ar = -1 * ar;
	return ar;
}
