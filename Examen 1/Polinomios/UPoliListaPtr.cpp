...
void PoliListaPtr::mostrar() {
	cout << lista->toStr() << endl;
		...
		if (k->vacia()) {
			...
			NodoL* b = NULL;
			NodoL* aux = k->primero()->sig;
			bool buscar = false;
			while (aux != NULL && buscar == false) {
				if (aux->sig->elemento == exp) {
					b = aux;
					buscar = true;
				}
			...
			if (b == NULL) {
				int grado = k->primero()->elemento;
				if (exp > grado)
					k->modifica(k->primero(), exp);
				NodoL* segundo = k->primero()->sig;
				k->inserta(segundo, coef);
				k->inserta(segundo, exp);
				nt++;
			}
			else {
				int elemento = b->elemento;
				k->modifica(b, elemento + coef);
				if (elemento + coef == 0) {
					NodoL* sig = b->sig; // nodo del exponente
					int gr = sig->elemento;
					k->suprime(b);
